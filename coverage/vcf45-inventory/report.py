...
reserved-key registries, the SSSOM mapping sets under ``mappings/`` and the
curated construct inventory in ``coverage/vcf45-inventory/inventory.json``; writes
``coverage/vcf45-inventory/generated/report.json`` and prints a summary.
...
Usage:  npm run coverage:report   (or: python3 coverage/vcf45-inventory/report.py)
"""
...
INLINE_MARKER = "also asserted inline in the vcf-core ontology modules"


def read_sssom(path: pathlib.Path) -> list[dict[str, str]]:
    """Read an SSSOM TSV, discarding the leading YAML-in-comments metadata block."""
...
def mapping_stats() -> dict:
    """Summarise the curated alignment set and the third-party sets beside it.

    The SSSOM TSVs are the authored source of truth; ``vcf-core-alignments.ttl``
    is generated from the first of them. Counting the TSVs therefore counts what
    a curator actually wrote, and keeps the manuscript's mapping figures on the
    same footing as its coverage figures.
    """
    registry = json.loads((ROOT / "mappings" / "registry.json").read_text())
    sets = {entry["file"]: entry for entry in registry["mappingSets"]}

    asserted_file = "vcf-core-alignments.sssom.tsv"
    rows = read_sssom(ROOT / "mappings" / asserted_file)
    inline = [r for r in rows if INLINE_MARKER in (r.get("comment") or "").lower()]
    targets: Counter[str] = Counter(r["object_id"].split(":", 1)[0] for r in rows)
    predicates: Counter[str] = Counter(r["predicate_id"] for r in rows)
    confidences = [float(r["confidence"]) for r in rows if r.get("confidence")]

    third_party = {}
    for name, entry in sets.items():
        if entry["status"] != "third-party":
            continue
        key = pathlib.PurePath(name).name.split("-to-")[0]
        third_party[key] = len(read_sssom(ROOT / "mappings" / name))

    return {
        "asserted": {
            "set": f"mappings/{asserted_file}",
            "module": f"mappings/{sets[asserted_file]['module']}",
            "mappings": len(rows),
            "distinctSubjects": len({r["subject_id"] for r in rows}),
            "distinctObjects": len({r["object_id"] for r in rows}),
            "alsoInlineInOntology": len(inline),
            "moduleOnly": len(rows) - len(inline),
            "withConfidence": len(confidences),
            "byTarget": dict(targets.most_common()),
            "byPredicate": dict(predicates.most_common()),
        },
        "thirdParty": {
            "event": registry["provenance"]["event"],
            "sets": len(third_party),
            "mappings": sum(third_party.values()),
            "bySet": dict(sorted(third_party.items())),
            "objectSource": "GA4GH VRS 1.3",
            "verified": registry["provenance"]["verified"],
        },
    }
...
def coverage_stats(graph: Graph) -> tuple[dict, list[str]]:
    ...
    for area in inventory["areas"]:
        ...
        area_tiers: Counter[str] = Counter()
        area_axes: Counter[str] = Counter()
        for construct in area["constructs"]:
            area_tiers[construct["evidenceTier"]] += 1
        ...
        per_area.append({
            ...
            "evidenceTiers": {tier: area_tiers.get(tier, 0)
                              for tier in ("query", "probe", "shape", "fixture")},
            "enforced": area_axes.get("enforced", 0),
        })

    total = sum(totals.values())
    full, partial = totals["full"], totals["partial"]
    logical = {
        "inventory": "coverage/vcf45-inventory/inventory.json",
        "constructs": total,
...
def main() -> int:
    ...
    requirements = ROOT / "coverage" / "methodology"
    result = subprocess.run([sys.executable, str(requirements / "scripts" / "assess.py"), "check"])
    if result.returncode:
        return result.returncode
    graph, per_module = load_ontology()
    coverage, missing = coverage_stats(graph)
    coverage["representation"] = json.loads((requirements / "generated" / "summary.json").read_text())
    ...
    print(f"Alignments asserted: {maps['asserted']['mappings']} SSSOM mappings "
          f"({maps['asserted']['alsoInlineInOntology']} also inline, "
          f"{maps['asserted']['moduleOnly']} module-only) -> "
          + ", ".join(f"{k} {v}" for k, v in maps["asserted"]["byTarget"].items()))
    print(f"Third-party sets ({maps['thirdParty']['event']}): "
          f"{maps['thirdParty']['mappings']} mappings in {maps['thirdParty']['sets']} sets "
          + ", ".join(f"{k}={v}" for k, v in maps["thirdParty"]["bySet"].items()))
