// Bounded fixture checks over examples/profile-comparison/: one synthetic VCF record
// represented in both sample profiles. This is neither a general VCF converter nor a
// SHACL validator; it asserts that both profiles recover the same source cells.
...
const fixture = name => new URL(`../examples/profile-comparison/${name}`, import.meta.url);
...
const destination = new URL('generated/profile-comparison.json', import.meta.url);
if (process.argv.includes('--check')) {
  assert.equal(readFileSync(destination, 'utf8'), output, 'Recorded profile-comparison verification is stale; run npm run validate:profiles');
} else {
